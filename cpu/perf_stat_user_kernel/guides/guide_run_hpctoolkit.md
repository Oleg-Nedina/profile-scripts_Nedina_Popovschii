# Guide: `run_hpctoolkit.sh`

This guide explains how to use the [run_hpctoolkit.sh](file:///home/olly/UNI/progetto_aca/profile-scripts_Nedina_Popovschii/cpu/perf_stat_user_kernel/scripts/run_hpctoolkit.sh) script to profile applications using statistical sampling via HPCToolkit.

---

## Overview

The [run_hpctoolkit.sh](file:///home/olly/UNI/progetto_aca/profile-scripts_Nedina_Popovschii/cpu/perf_stat_user_kernel/scripts/run_hpctoolkit.sh) script automates the multi-stage HPCToolkit workflow. It runs the statistical counter sampling (`hpcrun`), decodes loop architectures (`hpcstruct`), and correlates timing/hardware counter measurements with the application source files (`hpcprof`) to generate a performance database visualisable in hpcviewer.
...
1. **HPCToolkit and Spack**: HPCToolkit must be compiled and loaded (normally via `spack load hpctoolkit` inside the local `mudock_zen5` Spack environment).
...
```bash
GTK_THEME=Adwaita hpcviewer ./cpu/perf_stat_user_kernel/traces/hpctoolkit/database/
```

*Note: Forcing the GTK_THEME environment variable to `Adwaita` resolves rendering issues (such as black buttons or icons) on Linux dark setups.*
