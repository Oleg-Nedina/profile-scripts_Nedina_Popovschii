...
### 🔍 Risoluzione dei Problemi: Corrispondenze Multiple (Boost)

Se Spack restituisce l'errore:
```text
==> Error: boost@1.90.0 matches multiple packages.
```
Significa che sono presenti più installazioni di Boost 1.90.0 con hash differenti. In tal caso, per evitare errori di configurazione, è necessario specificare l'hash corretto dell'installazione attiva direttamente nel comando CMake (ad esempio `/m7yx265` o `/qnm4vei`):

```bash
# Esempio di configurazione con hash specifico per aggirare il problema:
$(spack location -i cmake@3.31.11)/bin/cmake .. \
  -DCMAKE_C_COMPILER=gcc-14 \
  -DCMAKE_CXX_COMPILER=g++-14 \
  -DCMAKE_POLICY_DEFAULT_CMP0144=NEW \
  ...
  -DMUDOCK_ENABLE_USER_EVENTS=ON \
  -DMUDOCK_ENABLE_PAPI=ON \
  -DCMAKE_PREFIX_PATH="$(spack location -i boost@1.90.0/m7yx265);$(spack location -i highway);$(spack location -i openbabel)"
```
